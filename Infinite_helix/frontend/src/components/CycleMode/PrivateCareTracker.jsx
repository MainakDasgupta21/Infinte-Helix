...
export default function PrivateCareTracker() {
  ...
  return (
    <div className="glass-card p-5 rounded-2xl border border-rose-400/15 space-y-4">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded-lg bg-rose-500/10">
            <HiOutlineShieldCheck className="w-4 h-4 text-rose-400" />
          </div>
          <div>
        ...
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-helix-mint/10 border border-helix-mint/25">
          <HiOutlineShieldCheck className="w-3 h-3 text-helix-mint" />
          <span className="text-[10px] text-helix-mint font-medium">Private</span>
        </div>
      </div>

      {/* Quick toggle: "I need reminders today" */}
      <div className="flex items-center justify-between bg-rose-500/5 border border-rose-400/15 rounded-xl p-3.5">
        <div>
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all ${
            cfg.activeToday
              ...
              : 'bg-helix-accent/10 border border-helix-accent/25 text-helix-accent hover:bg-helix-accent/20'
          }`}
        >
        ...
        <div className="flex items-center justify-between px-1">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="w-7 h-7 rounded-lg bg-helix-bg text-helix-muted hover:text-helix-text transition-colors flex items-center justify-center text-sm"
            >-</button>
      ...
      <div className="space-y-3">
        ...
        <input
          ...
          className="w-full bg-helix-bg/50 border border-helix-border/30 rounded-xl px-3.5 py-2.5 text-sm text-helix-text placeholder:text-helix-muted/50 focus:outline-none focus:border-helix-accent/40"
        />
        <button
          onClick={handleLog}
      ...
      {/* Today summary */}
      <div className="flex items-center justify-between bg-helix-bg/40 rounded-xl p-3 border border-helix-border/20">
        <div className="flex items-center gap-2">
        ...
        <div className="max-h-64 overflow-y-auto space-y-3 pr-1">
          {loading ? (
            ...
            <div className="text-center py-6">
              ...
              <p className="text-[10px] text-helix-muted/60 mt-1">Your logs will appear here</p>
            </div>
          ) : (
                ...
                <div className="space-y-1.5">
                  {grouped[date].map((log, i) => {
                    ...
                    return (
                      <div
                        key={`${date}-${i}`}
                        className="flex items-center gap-3 bg-helix-bg/30 rounded-lg px-3 py-2 border border-helix-border/15"
                      >
