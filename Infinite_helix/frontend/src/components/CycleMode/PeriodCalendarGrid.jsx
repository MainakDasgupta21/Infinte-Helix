    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <button
          ...
          onClick={goPrev}
          disabled={!canGoPrev}
          className="p-2 rounded-lg border border-helix-border/50 text-helix-text hover:bg-helix-border/30 disabled:opacity-30 disabled:cursor-not-allowed text-sm"
          aria-label="Previous month"
        >
          ←
        </button>
        <p className="text-sm font-semibold text-helix-text min-w-[10rem] text-center">{title}</p>
        <button
          type="button"
          onClick={goNext}
          disabled={!canGoNext}
      ...
      <div className="grid grid-cols-7 gap-1 text-center">
        {WEEKDAYS.map((w) => (
          <div key={w} className="text-[10px] sm:text-xs text-helix-muted py-1 font-medium">
            {w}
          </div>
        ))}
        {grid.map((d, i) => {
          ...
          return (
            <div
              ...
              className={`aspect-square rounded-lg text-sm font-medium flex flex-col items-center justify-center transition-all border
                ${isPeriod ? 'bg-rose-500/25 border-rose-400/40 text-helix-text' : 'border-transparent text-helix-text'}
                ${isStart ? 'ring-2 ring-rose-400 ring-offset-1 ring-offset-helix-surface' : ''}
                ${isToday && !isPeriod ? 'ring-1 ring-helix-border/50 bg-helix-border/30' : ''}
                ${isToday && isPeriod ? 'ring-1 ring-helix-accent/20' : ''}
              `}
      ...
      <div className="flex items-center justify-center gap-4 text-[11px] text-helix-muted flex-wrap">
        ...
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded ring-1 ring-helix-border/50 bg-helix-border/30" /> Today
        </span>
      </div>
    </div>
  );
