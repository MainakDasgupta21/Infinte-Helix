    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <button
          ...
          onClick={goPrev}
          disabled={!canGoPrev}
          className="p-2 rounded-lg border border-helix-border text-helix-text hover:bg-helix-bg/60 disabled:opacity-30 disabled:cursor-not-allowed text-sm"
          ...
          return (
            <div
              ...
              className={`aspect-square rounded-lg text-sm font-medium flex flex-col items-center justify-center transition-all border
                ...
                ${isStart ? 'ring-2 ring-rose-400 ring-offset-1 ring-offset-helix-bg' : ''}
                ${isToday && !isPeriod ? 'ring-1 ring-helix-border bg-helix-bg/40' : ''}
                ${isToday && isPeriod ? 'ring-1 ring-helix-accent/50' : ''}
              `}
      ...
      <div className="flex items-center justify-center gap-4 text-[11px] text-helix-muted flex-wrap">
        ...
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded ring-1 ring-helix-border bg-helix-bg/40" /> Today
        </span>
      </div>
    </div>
  );
