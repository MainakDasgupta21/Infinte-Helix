...
export default function BreakBalance({ breaks }) {
  ...
  return (
    <div className="bento-card h-full flex flex-col">
        ...
        {Array.from({ length: breaks.suggested }).map((_, i) => (
          <span
            ...
            className={`h-3 w-3 rounded-full transition-all duration-500 ${
              i < breaks.taken ? 'bg-blue-600 shadow-[0_0_8px_rgba(59,130,200,0.3)]' : 'bg-helix-border/30'
            }`}
          />
        ))}
      </div>
      ...
      <div className="w-full h-2 bg-helix-border/30 rounded-full overflow-hidden mb-5">
        <div
      ...
      <div className="space-y-3 flex-1">
        ...
        {remaining > 0 && (
          <div className="rounded-2xl px-4 py-3 bg-amber-500/10 text-amber-600 text-xs font-medium">
