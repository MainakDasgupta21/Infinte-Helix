...
const TYPE_STYLES = {
  hydration: { label: 'Hydration', bg: 'bg-blue-500/10', text: 'text-blue-600', emoji: '\uD83D\uDCA7' },
  stretch:   { label: 'Move!', bg: 'bg-emerald-500/10', text: 'text-emerald-600', emoji: '\uD83E\uDDD8' },
  posture:   { label: 'Posture', bg: 'bg-emerald-500/10', text: 'text-emerald-600', emoji: '\uD83E\uDEBB' },
  eyes:      { label: 'Eye Rest', bg: 'bg-helix-accent/10', text: 'text-helix-accent', emoji: '\uD83D\uDC41\uFE0F' },
  meeting:   { label: 'Meeting', bg: 'bg-rose-500/10', text: 'text-rose-500', emoji: '\uD83C\uDFAF' },
  emotional: { label: 'Vibe Check', bg: 'bg-helix-accent/10', text: 'text-helix-accent', emoji: '\uD83D\uDC9C' },
  winddown:  { label: 'Log Off!', bg: 'bg-helix-accent/10', text: 'text-helix-accent', emoji: '\uD83C\uDF19' },
  morning:   { label: 'Morning', bg: 'bg-amber-500/10', text: 'text-amber-600', emoji: '\u2600\uFE0F' },
  streak:    { label: 'Achievement', bg: 'bg-amber-500/10', text: 'text-amber-600', emoji: '\uD83C\uDFC6' },
};
    ...
    <div className="bento-card h-full flex flex-col">
      <div className="flex items-center justify-between mb-5">
        ...
        {activeNudges.length > 0 && (
          <span className="text-[11px] bg-helix-border/30 text-helix-muted px-2.5 py-1 rounded-xl font-medium tabular-nums">
      ...
      <div className="space-y-3 max-h-64 overflow-y-auto pr-1 flex-1">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2.5">
                    ...
                    <span className="text-[11px] text-helix-muted">{nudge.time}</span>
                  </div>
                </div>
                <button type="button" onClick={() => dismissNudge(nudge.id)} className="text-helix-muted/50 hover:text-helix-muted p-1 transition-colors">
        ...
        {showContextual && (
          <div className="rounded-2xl bg-helix-accent/10 p-4 flex items-start gap-3">
            <span className="text-[10px] uppercase tracking-wider font-semibold text-helix-accent shrink-0 mt-0.5">Tip</span>
            <p className="text-sm text-helix-text leading-relaxed flex-1 min-w-0">{contextual.text}</p>
            <button
              type="button"
              onClick={dismissContext}
              className="shrink-0 text-[11px] font-medium px-3 py-1.5 rounded-xl bg-helix-surface/60 text-helix-muted hover:text-helix-text hover:bg-helix-surface/80 transition-colors"
            >
              Done
            </button>
          </div>
        ...
        {pastNudges.length > 0 && (
          <div className="pt-3" style={{ borderTop: '1px solid rgba(0,0,0,0.04)' }}>
            <p className="text-[11px] text-helix-muted mb-2">Earlier</p>
        ...
        {nudges.length === 0 && !showContextual && (
          <div className="text-center py-10 text-helix-muted">
            {workHours ? (
